...
namespace EF.Estoques.Domain.Test.Model
{
    ...
    public class EstoqueFixture : IDisposable
    {
        ...
        public MovimentacaoEstoque GerarMovimentacao(Guid estoqueId, TipoMovimentacaoEstoque? tipoMovimentacao = null, OrigemMovimentacaoEstoque? origemMovimentacao = null)
        {
            return GerarMovimentacoes(1, estoqueId, tipoMovimentacao, origemMovimentacao).FirstOrDefault()!;
        }

        public List<MovimentacaoEstoque> GerarMovimentacoes(int quantidade, Guid estoqueId, TipoMovimentacaoEstoque? tipoMovimentacaoParam = null, OrigemMovimentacaoEstoque? origemMovimentacaoParam = null)
        {
            var tipoMovimentacao = tipoMovimentacaoParam ?? GetRandomEnum<TipoMovimentacaoEstoque>(Enum.GetValues(typeof(TipoMovimentacaoEstoque)));

            OrigemMovimentacaoEstoque origemMovimentacao;
            if (origemMovimentacaoParam is null)
            {
                var origemEntrada = new OrigemMovimentacaoEstoque[] { OrigemMovimentacaoEstoque.Compra, OrigemMovimentacaoEstoque.CancelamentoVenda };
                var origemSaida = new OrigemMovimentacaoEstoque[] { OrigemMovimentacaoEstoque.Venda };
            ...
            var itens = new Faker<MovimentacaoEstoque>("pt_BR")
                .CustomInstantiator(f => new MovimentacaoEstoque(estoqueId, Guid.NewGuid(), f.Random.Number(5), tipoMovimentacao, origemMovimentacao!, DateTime.Now));
