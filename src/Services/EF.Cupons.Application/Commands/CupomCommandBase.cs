...
namespace EF.Cupons.Application.Commands
{
    public class CupomCommandBase : CommandHandler
    {
        ...
        protected async Task<bool> ValidarCupom(Guid cupomId, CancellationToken cancellationToken, bool validarVigencia = true)
        {
        ...
        {
            var listaCupom = await _cupomRepository.BuscarCupomVigenteEmPeriodo(cupom.CodigoCupom, cupom.DataInicio, cupom.DataFim, cancellationToken);
