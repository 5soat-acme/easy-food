...
public class CarrinhoService(ICarrinhoRepository carrinhoRepository, IUserApp user, IMapper mapper, IProdutoService produtoService)
    : ICarrinhoService
{
    ...
    public async Task<OperationResult<Item>> AdicionarItemCarrinho(AdicionarItemDto itemDto)
    {
        var item = await ObterItemValidado(itemDto.ProdutoId, itemDto.Quantidade);
    ...
    private async Task<Item> ObterItemValidado(Guid produtoId, int quantidade)
    {
        var item = await produtoService.ObterItemPorProdutoId(produtoId);
        //TODO: Validar item
        return item;
    }
