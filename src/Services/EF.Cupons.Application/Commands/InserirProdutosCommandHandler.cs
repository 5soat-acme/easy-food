    ...
    IRequestHandler<InserirProdutosCommand, CommandResult>
{
    ...
    private async Task<IList<CupomProduto>> GetProdutos(InserirProdutosCommand command,
        CancellationToken cancellationToken)
    {
        ...
        {
            var prodExiste =
                await _cupomRepository.BuscarCupomProduto(command.CupomId, p, cancellationToken) is not null;
            if (!prodExiste)
